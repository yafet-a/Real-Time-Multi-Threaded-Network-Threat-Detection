...
void *worker(void *arg)
{
  ...
  while (1)
  {
    ...
    sleep(0.001); //encourage contention/ force context switching

    printf("Thread %ld is waiting for a packet\n", pthread_self());

    // Check if work items in queue  (process)
    while (work_queue_head == NULL)
    {
      printf("Thread %ld is waiting for work\n", pthread_self());
    ...
    printf("Thread %ld picked up a packet from the queue\n", pthread_self());
              ...
              int verbose)
{
  ...
  // free(packet_item);
  printf("Dispatching work to the queue\n");
